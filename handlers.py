...
# --- 3. КАТАЛОГ ПЕСЕН (МЕНЮ С КНОПКАМИ) ---
...
def play_song_with_buttons(call):
    ...
    if file_id:
        ...
        bot.answer_callback_query(call.id, text="Ошибка файла", show_alert=True)

# --- 4. ГОЛОСОВАНИЕ И ОТЗЫВЫ ---
...
def handle_vote(call):
    ...
    if success:
        ...
        bot.answer_callback_query(call.id, text="❌ Ошибка", show_alert=True)
...
def start_review(call):
    ...
    bot.answer_callback_query(call.id)
    bot.send_message(user_id, "✍️ Напиши свой отзыв об этом треке:\n(Чтобы отменить, напиши /cancel)")
